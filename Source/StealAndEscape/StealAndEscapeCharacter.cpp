...
Updated: Added manual grab item system with candidate list.
The player maintains a TArray of all StealableItems currently in overlap range.
GrabPressed() checks if the list is non-empty before playing the grab animation.
CollectNearbyItem() is called by the grab animation notify at the hand-contact frame
and picks the closest item from the candidate list. This correctly handles cases
where multiple items are in range at the same time.
...
/* Custom Method for Grab functionality using G key
   Updated: Now checks if there are any candidate items in range before playing
