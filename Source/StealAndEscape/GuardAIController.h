...
public:
	...
	/* ResetToPatrol is exposed to blueprint so we can call it from outside this class
	   for example form  game mode or restart logic. It clears all chase and
	   invesigation state then sends the guard back to closest patrol point so the
	   guard goes back to its normal patroling behavior again .
	*/
...
private:
	...
	/* Below this is for the guard to move between patrol points*/
